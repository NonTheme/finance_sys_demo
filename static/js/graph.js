        ...
        if (second === 'search1') {
            $.ajax({
                ...
                success: function (json_str) {
                    ...
                    console.log('success!');
                    $('#main-parent').empty();
                    $('#main-parent').append('<div id="main" style="width:1200px;height: 800px;"></div>');
...
function crt_table(json_list) {
    var data = json_list;
    $('#year-div').empty();
    $('#main').empty().append('<div class="table-responsive"><table class="table table-bordered table-hover table-condensed" id="answer-table"></table></div>')
    var str = '';
    for (var i in data) {
        str += "<tr>";
        for (var j in data[i]) {
